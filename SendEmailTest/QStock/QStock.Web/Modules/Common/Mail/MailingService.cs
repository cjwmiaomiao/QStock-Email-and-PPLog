...
{
    using Entities;
    using QStock.Common.Repositories;
    using Serenity;
    using Serenity.Data;
    using System;
    ...
    public class MailingService
    {
        ...
        private MailMessage BuildMessageHasAttachment(MailRow mail)
        {
            ...
            MemoryStream ms = new MemoryStream(mail.Attachments);
            message.Attachments.Add(new Attachment(ms, mail.Subject + ".xlsx"));
        ...
        public void EnqueueHasAttachment(IDbConnection connection, MailMessageModel message, Guid? uid = null)
        {
            var mail = new MailRow();
            mail.Uid = uid ?? Guid.NewGuid();
            mail.Subject = message.Subject;
            ...
            mail.RetryCount = 0;
            mail.MailFrom = message.From != null ? message.From.ToString() : null;
            mail.Attachments = message.AttachmentData;
            mail.ModuleName = message.ModuleName;
            if (message.To.Count > 0)
                mail.MailTo = string.Join(";", message.To.Select(x => x.ToString()));
            if (message.CC.Count > 0)
                mail.Cc = string.Join(";", message.CC.Select(x => x.ToString()));
        ...
        public bool SendById(IDbConnection connection, long mailId)
        {
            ...
            try
            {
                ...
                var message = BuildMessageHasAttachment(mail);
